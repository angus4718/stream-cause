#!/usr/bin/env python3
"""Build a single packed trades binary over a date range for the C++ pipeline."""
...
def session_ns(date: datetime.date) -> tuple[int, int]:
    ...
    return (int(open_dt.timestamp() * 1_000_000_000), int(close_dt.timestamp() * 1_000_000_000))
...
def load_sym_events(sym: str, inst_id: int, sessions: list[tuple[datetime.date, int, int]]) -> np.ndarray:
...
def main():
    ...
    out_path = os.path.join(args.out_dir, f"allstock_trades_{start_compact}_{end_compact}.bin")
