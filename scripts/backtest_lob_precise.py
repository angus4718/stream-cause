#!/usr/bin/env python3
"""
Queue-aware MM backtest using full LOB reconstruction (lob.py Book/Market).

Fill model
----
At each 30-second tick start the MM places a virtual limit bid at
  bid_px = round_tick(mid - mm_hs)
and a virtual limit ask at
  ask_px = round_tick(mid + mm_hs)
Queue-ahead = existing book size at that price level.
Fills when cumulative volume at that price level exceeds queue_ahead.

P&L per fill = fill_shares * (half_spread_posted - |Deltamid|)

Strategies tested
----
sel      : Hawkes-select (max(k_tod*TOD, k_h*lambda) for high-cross_in instruments)
tod      : TOD baseline (k_tod * TOD(s))
fix      : Fixed spread (k_tod * 1.0, IS-calibrated optimal constant)
qjn      : Qjoin -- always post at best bid/ask
hgt      : Hawkes-gate -- post at best bid/ask when lambda <= tau_gate, else sit out
hmin     : Hawkes-min -- min(k_h*lambda, k_tod*TOD); never wider than TOD
hfg      : Hawkes-fix-gate -- post at Fixed when lambda <= tau_gate, else sit out
htg      : Hawkes-tod-gate -- post at TOD when lambda <= tau_gate, else sit out
hinv     : Hawkes-inv -- k_fix^2/(k_h*lambda+epsilon) clamped to [0.3*k_fix, 3*k_fix]
hfixfl   : Hawkes-fix-floor -- max(k_h*lambda, k_fix) like Hselect but floor=Fixed
"""
...
POLICIES = ["sel", "tod", "fix", "qjn", "hgt",
            "hmin", "hfg", "htg", "hinv", "hfixfl"]
(POL_SEL, POL_TOD, POL_FIX, POL_QJN, POL_HGT,
 POL_HMIN, POL_HFG, POL_HTG, POL_HINV, POL_HFIXFL) = range(10)
NPOL = len(POLICIES)
...
def ts_to_et_date_time(ts_ns: int):
    dt_utc = datetime.datetime.utcfromtimestamp(ts_ns / 1e9).replace(
...
def load_lambda_series(output_dir: Path, date_str: str) -> np.ndarray:
    ...
    paths = sorted(date_dir.glob("lambda_*.bin"),
                   key=lambda p: int(re.search(r"lambda_(\d+)\.bin", p.name).group(1)))
    if not paths:
        return np.empty((0, 0))
    vecs = []
    for p in paths:
...
def trading_days(s: datetime.date, e: datetime.date) -> list:
    US_HOLIDAYS = {datetime.date(2025, 11, 27), datetime.date(2025, 11, 28),
...
def simulate_instrument_oos(
    ...
    k_h: float,
    k_tod: float,
    k_fix: float,        # Fixed half-spread (dollars)
    use_floor: bool,
    tau_gate: float,     # Hawkes-gate sit-out threshold
    mm_size: int = 100,
) -> list:
    ...
    for d in oos_days:
        ...
        mm_hinv  = np.clip(k_fix * k_fix / (k_h * lam_i + lam_eps),
                           0.3 * k_fix, 3.0 * k_fix)
...
def main():
    ...
    print("Computing IS cross_in from alpha matrices...")
    ...
    print(f"  {use_floor.sum()}/{N} instruments use Hawkes-floor")

    all_day_results = []
    print(f"\nSimulating OOS ({len(oos_days)} days) for {N} instruments...")
    ...
    for i in range(N):
        ...
        if sym not in cal_k.index:
            print(f"  {sym}: skip (not in calibrated_k)")
            continue
                ...
                for f in sorted(sym_dir.glob(f"XNAS_ITCH_{sym}_mbo_*.dbn.zst")):
                    ...
                    if m:
                        ...
                        if f_start <= oos_month_start and f_end >= oos_month_end:
                            ...
                            break
            if fallback is None:
                print(f"  {sym}: skip (file not found: {dbn_file.name})")
                continue
            print(f"  {sym}: using fallback file {fallback.name}")
            dbn_file = fallback

        print(f"  {sym}: use_floor={uf_i}  k_h={k_h_i:.2e}  k_tod={k_tod_i:.2e}  "
              f"tau={tau_gate_i:.2e}", flush=True)
        try:
        ...
        print(f"    -> {len(day_res)} days processed")
