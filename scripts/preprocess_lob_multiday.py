#!/usr/bin/env python3
"""
Multi-day LOB preprocessor (parallel).

Each worker handles one symbol's complete file set, replays the order book,
and writes per-day temp binary files under tmp/lob/<sym>/<YYYYMMDD>.bin.
The main process then merges all symbols' temp files into the final per-day
LOB binaries at data/lob/<YYYYMMDD>_allstock_lob.bin.

Output: data/lob/<YYYYMMDD>_allstock_lob.bin   (one per trading day)
        data/allstock_id_map.json

Usage:
    python scripts/preprocess_lob_multiday.py \
        [--start YYYY-MM-DD] [--end YYYY-MM-DD] [--workers 16]
"""
...
def process_symbol(sym: str, inst_id: int,
    ...
    for file_start, file_end, filepath in file_list:
        ...
        print(f"  [{sym}] {os.path.basename(filepath)} -> {len(days_in_file)} days",
              flush=True)
        ...
        for d in days_in_file:
            print(f"  [{sym}] {d}: {counts[d]:,} records", flush=True)
...
def main():
    ...
    print(f"  {len(target_days) - len(todo_days)} days already done, "
          f"processing {len(todo_days)}")
    if not todo_days:
        print("All done.")
        return
    ...
    print(f"\nRunning {len(worker_syms)} symbol workers (max {args.workers} parallel)"
          + (f" [only_syms={args.only_syms}]" if args.only_syms else "") + "...")
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as pool:
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                total = sum(all_counts[sym].values())
                print(f"  [OK] {sym}: {total:,} records total", flush=True)
            except Exception as e:
                print(f"  [FAIL] {sym}: ERROR {e}", flush=True)
                import traceback; traceback.print_exc()

    # Merge per-symbol temp files into per-day LOB binaries
    print("\nMerging symbol temp files into per-day LOB binaries...")
    for d in todo_days:
        ...
        print(f"  {d}: {n_total:,} records  ({mb:.1f} MB)")
