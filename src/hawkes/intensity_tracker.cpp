...
void IntensityTracker::set_params(const Eigen::MatrixXd& alpha, const Eigen::VectorXd& mu) {
    alpha_ = alpha;
    mu_ = mu;
}
...
const Eigen::VectorXd& IntensityTracker::on_event(const MarketEvent& ev) {
    ...
    double t_j = static_cast<double>(ev.ts_event) * 1e-9;

    if (last_t_[j] >= 0.0) {
        double decay = std::exp(-beta_ * (t_j - last_t_[j]));
        for (int k = 0; k < n_; ++k) {
